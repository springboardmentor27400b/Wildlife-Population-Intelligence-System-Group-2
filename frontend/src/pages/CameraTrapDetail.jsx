...
import { Camera, MapPin, Calendar, Compass, ArrowLeft } from 'lucide-react';
import { getCameraTrap } from '../api/cameraTraps';
...
export const CameraTrapDetail = () => {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      setLoading(true);
      try {
        const camData = await getCameraTrap(id);
        setCamera(camData);
        
        if (camData.site_id) {
          const siteData = await getMonitoringSite(camData.site_id);
          setSite(siteData);
        }
      } catch (err) {
        console.error(err);
        setToastMsg({ text: 'Failed to retrieve camera trap details.', type: 'error' });
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-650 dark:text-slate-400">
              Serial Number: <span className="font-mono text-slate-800 dark:text-slate-200">{camera.serial_number}</span>
            </p>
            
            <div className="flex items-center gap-2 text-xs text-slate-450 pt-2">
              <Calendar className="w-4 h-4" />
              <span>Registered: {formatDateTime(camera.created_at)}</span>
            </div>
          </div>
        </div>
      </Card>
