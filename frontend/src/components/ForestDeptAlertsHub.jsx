...
export default function ForestDeptAlertsHub() {
  const [alerts, setAlerts] = useState([]);
  const [summary, setSummary] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [activeTab, setActiveTab] = useState('ALL'); // ALL, endangered_species, device_alert, habitat_degradation, population_decline, conservation_notification
  ...
  const fetchAlertsData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch ForestDept alerts:', err);
      setError('Failed to load operational hardware & habitat alerts.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const categoryTabs = [
    { key: 'ALL', label: 'All Forest Alerts', count: summary?.total || 0, icon: Bell },
    { key: 'endangered_species', label: 'Endangered Sightings', count: summary?.counts_by_type?.endangered_species || 0, icon: ShieldAlert },
    { key: 'device_alert', label: 'Device Telemetry', count: summary?.counts_by_type?.device_alert || 0, icon: Cpu },
    ...
    { key: 'conservation_notification', label: 'Park Advisories', count: summary?.counts_by_type?.conservation_notification || 0, icon: Compass },
  ];
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-emerald-950/60 border border-emerald-800/60 text-emerald-400 flex items-center gap-1">
              ...
              Forest Dept Scope (5 Alert Categories)
            </span>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-zinc-950 border border-zinc-800 text-zinc-400">
              Hardware Maintenance & Park Telemetry
            </span>
          </div>
          <h2 className="text-xl font-extrabold text-zinc-100 mt-1 flex items-center gap-2">
            <span>Forest Department Operational Alerts Hub</span>
            <Trees size={20} className="text-emerald-400" />
          </h2>
          <p className="text-xs text-zinc-400 mt-1">
            Real-time infrastructure management monitoring camera trap connectivity, acoustic sensor health, satellite habitat stress, and population shifts.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Device Hardware Alerts</p>
            <p className="text-xl font-extrabold text-blue-400 mt-0.5">{summary?.counts_by_type?.device_alert || 0}</p>
          </div>
          <div className="h-9 w-9 rounded-lg bg-blue-950/50 border border-blue-900/40 flex items-center justify-center text-blue-400">
            <Cpu size={18} />
          </div>
        </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Habitat Warnings</p>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Total Operational Logs</p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder="Search device or habitat alerts..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="py-12 flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-zinc-400 font-medium">Scanning park hardware & GIS telemetry...</p>
        </div>
      ) : error ? (
        ...
        <div className="py-12 bg-zinc-950/40 border border-zinc-800/60 rounded-xl text-center space-y-2">
          ...
          <p className="text-sm font-bold text-zinc-300">All Field Infrastructure Operational</p>
          <p className="text-xs text-zinc-500 max-w-sm mx-auto">
            All camera traps, acoustic sensors, habitat satellite indices, and population parameters are operating cleanly.
          </p>
        </div>
      ) : (
            ...
            >
              ...
              <div className="flex items-center space-x-2 shrink-0 self-end md:self-center">
                {alert.alert_type === 'device_alert' && (
                  <span className="text-[11px] font-bold text-blue-400 bg-blue-950/60 border border-blue-800/60 px-2.5 py-1 rounded-lg flex items-center gap-1">
                    <Wrench size={12} /> Dispatch Maintenance
                  </span>
                )}
                {!alert.is_read && (
                  <button
