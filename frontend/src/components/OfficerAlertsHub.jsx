...
export default function OfficerAlertsHub() {
  ...
  const fetchAlertsData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch Officer alerts:', err);
      setError('Failed to load tactical wildlife enforcement alerts.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const categoryTabs = [
    { key: 'ALL', label: 'All Patrol Alerts', count: summary?.total || 0, icon: Bell },
    ...
    { key: 'habitat_degradation', label: 'Habitat Stress', count: summary?.counts_by_type?.habitat_degradation || 0, icon: Sun },
    { key: 'conservation_notification', label: 'Field Directives', count: summary?.counts_by_type?.conservation_notification || 0, icon: Compass },
  ];
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-red-950/60 border border-red-800/60 text-red-400 flex items-center gap-1">
              ...
              Officer Scope (4 Alert Categories)
            </span>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-zinc-950 border border-zinc-800 text-zinc-400">
              Wildlife Protection & Field Enforcement
            </span>
          </div>
          <h2 className="text-xl font-extrabold text-zinc-100 mt-1 flex items-center gap-2">
            <span>Wildlife Officer Tactical Alerts Hub</span>
            <Siren size={20} className="text-red-400" />
          </h2>
          <p className="text-xs text-zinc-400 mt-1">
            Real-time enforcement telemetry covering endangered species detections, anti-poaching patrol dispatch, population dips, and environmental stress.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Endangered Detections</p>
            <p className="text-xl font-extrabold text-red-500 mt-0.5">{summary?.counts_by_type?.endangered_species || 0}</p>
          </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Total Patrol Logs</p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder="Search tactical alerts..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="py-12 flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-zinc-400 font-medium">Scanning enforcement radar...</p>
        </div>
      ) : error ? (
        ...
        <div className="py-12 bg-zinc-950/40 border border-zinc-800/60 rounded-xl text-center space-y-2">
          ...
          <p className="text-sm font-bold text-zinc-300">All Patrol Sectors Secure</p>
          <p className="text-xs text-zinc-500 max-w-sm mx-auto">
            No active threat alerts or tactical anti-poaching dispatches pending across reserve sectors.
          </p>
        </div>
      ) : (
          ...
          {filteredAlerts.map(alert => (
            ...
            >
              ...
              <div className="flex items-center space-x-2 shrink-0 self-end md:self-center">
                {alert.alert_type === 'endangered_species' && (
                  <span className="text-[11px] font-bold text-red-400 bg-red-950/60 border border-red-800/60 px-2.5 py-1 rounded-lg flex items-center gap-1">
                    <Navigation size={12} /> Deploy Patrol
                  </span>
                )}
                {!alert.is_read && (
                  <button
