...
export default function ResearcherAlertsHub() {
  const [alerts, setAlerts] = useState([]);
  const [summary, setSummary] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [activeTab, setActiveTab] = useState('ALL'); // ALL, endangered_species, population_decline, habitat_degradation, conservation_notification
  ...
  const fetchAlertsData = async () => {
    ...
    try {
      ...
      console.error('Failed to fetch Researcher alerts:', err);
      setError('Failed to load scientific research alerts.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const categoryTabs = [
    { key: 'ALL', label: 'All Scientific Alerts', count: summary?.total || 0, icon: Bell },
    { key: 'endangered_species', label: 'Endangered Sightings', count: summary?.counts_by_type?.endangered_species || 0, icon: ShieldAlert },
    { key: 'population_decline', label: 'Population Dynamics', count: summary?.counts_by_type?.population_decline || 0, icon: TrendingDown },
    { key: 'habitat_degradation', label: 'Habitat Telemetry', count: summary?.counts_by_type?.habitat_degradation || 0, icon: Sun },
    { key: 'conservation_notification', label: 'Scientific Advisories', count: summary?.counts_by_type?.conservation_notification || 0, icon: Compass },
  ];
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl space-y-6">
        ...
        <div>
          <div className="flex items-center space-x-2">
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-blue-950/60 border border-blue-800/60 text-blue-400 flex items-center gap-1">
              ...
              Researcher Scope (4 Alert Categories)
            </span>
            <span className="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-zinc-950 border border-zinc-800 text-zinc-400">
              Ecological Research & Population Dynamics
            </span>
          </div>
          <h2 className="text-xl font-extrabold text-zinc-100 mt-1 flex items-center gap-2">
            <span>Research & Scientific Telemetry Alerts Hub</span>
            <Microscope size={20} className="text-blue-400" />
          </h2>
          <p className="text-xs text-zinc-400 mt-1">
            Real-time scientific telemetry tracking species demographic shifts, capture-recapture dips, satellite GIS vegetation stress, and benchmark advisories.
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Population Dips</p>
            <p className="text-xl font-extrabold text-amber-400 mt-0.5">{summary?.counts_by_type?.population_decline || 0}</p>
          </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Habitat Stress Alerts</p>
            <p className="text-xl font-extrabold text-yellow-400 mt-0.5">{summary?.counts_by_type?.habitat_degradation || 0}</p>
          </div>
          <div className="h-9 w-9 rounded-lg bg-yellow-950/50 border border-yellow-900/40 flex items-center justify-center text-yellow-400">
            <Sun size={18} />
          </div>
        </div>
        ...
        <div className="bg-zinc-950/60 border border-zinc-800/80 rounded-xl p-3.5 flex items-center justify-between">
          <div>
            <p className="text-[10px] font-bold uppercase tracking-wider text-zinc-500">Total Telemetry Logs</p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder="Search scientific research alerts..."
            value={searchQuery}
      ...
      {loading ? (
        <div className="py-12 flex flex-col items-center justify-center space-y-3">
          ...
          <p className="text-xs text-zinc-400 font-medium">Scanning research telemetry & GIS feeds...</p>
        </div>
      ) : error ? (
        ...
        <div className="py-12 bg-zinc-950/40 border border-zinc-800/60 rounded-xl text-center space-y-2">
          ...
          <p className="text-sm font-bold text-zinc-300">All Research Benchmarks Normal</p>
          <p className="text-xs text-zinc-500 max-w-sm mx-auto">
            No scientific population drops or habitat degradation warnings currently logged across study sites.
          </p>
        </div>
      ) : (
          ...
          {filteredAlerts.map(alert => (
            ...
            >
              ...
              <div className="flex items-center space-x-2 shrink-0 self-end md:self-center">
                <span className="text-[11px] font-bold text-emerald-400 bg-emerald-950/60 border border-emerald-800/60 px-2.5 py-1 rounded-lg flex items-center gap-1">
                  <FileSpreadsheet size={12} /> Export Telemetry
                </span>
                {!alert.is_read && (
                  <button
