...
def check_admin_permission(current_user: User):
    if get_role_name(current_user).lower() != "administrator":
        raise HTTPException(status_code=403, detail="Only an Administrator can verify observation records.")
...
async def get_observations(current_user: User = Depends(get_current_user)):
    return await ObservationRecord.find_all().sort("-observed_at").to_list()
